...
One of most commonly used systems for building automation systems in Germany and Europe is \Gls{knx} or Konnex. Formerly known as the \gls{eib}, \gls{knx} is a \alert{conglomerate} of multiple building automation vendors, co-operating in the KNX Association. \todo{link as footnote?}
...
\todo{mention, that knx is only one example for bas. here used...}
...
The transport layer defines multiple transport layer services, which are encoded in the \gls{tpci} (cf. Table~\ref{tab:background:bas:knx:comm:tpci-apci}) and can be grouped into 5 categories or modes: \hint{grouped does not seems to be the right word here}
...
\subsection{KNX Management Procedures}
...
With \gls{knx} being so widely used, it is also deployed in security sensitive areas like access control, ventilation, or \todo{something third}.
