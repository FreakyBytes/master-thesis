\def\pgfsysdriver{pgfsys-dvipdfm.def}
\documentclass{beamer}
\providecommand\insertframetitle{}

\usepackage[english]{babel}
\usepackage{environ}
\usepackage{graphicx}
\usepackage[tikz]{bclogo}
\usepackage{tikz}
\usepackage{listings}
\usepackage{multimedia}
\usepackage{hyperref}
\usetikzlibrary{calc}
\usepackage[utf8]{inputenc}
\usepackage[overlay,absolute]{textpos}
\usepackage{xpatch}
\usepackage{csquotes}
\usepackage{comment}
\usepackage{mathtools}
\usepackage{pgfpages}

\usepackage{booktabs}				% improvements to LaTeX tables
\usepackage{tabularx}				% better tables
\usepackage{multirow}				% for cells spanning multiple rows
\usepackage{multicol}				% seems to be necessary for multirow?

\beamertemplatenavigationsymbolsempty
\makeatletter
\def\beamer@framenotesbegin{% at beginning of slide
	\usebeamercolor[fg]{normal text}
	\gdef\beamer@noteitems{}% 
	\gdef\beamer@notes{}% 
}
\makeatother

\xpatchcmd{\beamer@part}{\Hy@writebookmark{\the\c@section}{#1}{Outline\the\c@part}{1}{toc}}{}{}{}
\newcommand{\srcsize}{\@setfontsize{\srcsize}{5pt}{5pt}}
\usepackage[uni, footuni, headframelogo]{./unirostock/beamerthemeRostock}
\footinstitute{Institute of Computer Science}
%\footinstitute{Institut für Informatik}

\input{../../tex/shortcuts.tex}
\input{metadata.tex}

\title[\thetitle]{\themaintitle}
\subtitle{for Field Bus Networks\\in Building Automation Systems}
\author{\theauthor}
\date{2018-04-16}

\usepackage[backend=bibtex, style=numeric,
	citestyle=numeric,
	sortcites=true, block=none,	indexing=true,
	citereset=none, isbn=false,
	url=true, doi=true, natbib=false,
	hyperref=true]{biblatex}					% even better citation support
\addbibresource{bas-sec.bib}
\addbibresource{standards.bib}

\makeatletter
\makeatother
\headLogoTop{standard} % Main-Logo (``hafen'' or ``standard'')
\secondHeadLogoTop{20}{./res/iuk-logo-farbe.pdf} % Optional: Image for a second logo in the header
%\showNaviSymbols % show Navi-symbols in the footer
%\showNaviDots %show Navi-Dots in the header
%\withParts %toogle Pagenumbering with parts on
%\printmode{handout} % Remove decoration

% Show notes page with slide plus note
\setbeameroption{show notes}
\setbeameroption{show notes on second screen=right}
% covered points do not disappear, but get transparent
\setbeamercovered{transparent} 

\begin{document}
{
	\setbeamertemplate{footline}[titlewithoutnumber] %hg%no numbers on first page
	\maketitle
	\note{
		Hello, my name is Martin Peters.\\
		Analysis of Distributed In-Band Monitoring Messages for Field Bus Networks in Building Automation Systems\\
		Entwicklung von Verfahren zur Analyse von Netflows in Feldbussen in der Gebäudeautomation
	}
}

%\tableofcontents

\input{intro.tex}
\input{content.tex}
\input{outro.tex}
\input{backup-slides.tex}


\end{document}
